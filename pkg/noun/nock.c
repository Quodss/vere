...
static u3_noun
_n_melt(u3_noun ops, c3_w_tmp* byc_w, c3_w_tmp* cal_w,
        c3_w_tmp* reg_w, c3_w_tmp* lit_w, c3_w_tmp* mem_w)
{
  c3_w_tmp len_w = (c3_w_tmp)u3qb_lent(ops),
       i_w = len_w - 1,
       a_w;
  c3_y cod_y;
 ...
 */
static u3n_prog*
_n_prog_new(c3_w_tmp byc_w, c3_w_tmp cal_w,
            c3_w_tmp reg_w, c3_w_tmp lit_w, c3_w_tmp mem_w)
{
  c3_w_tmp cab_w = (sizeof(u3j_site) * cal_w),
...
{
  c3_w_tmp cab_w = sizeof(u3j_site) * sep_u->cal_u.len_w,
 ...
 */
static void
_n_prog_asm_inx(c3_y* buf_y, c3_w_tmp* i_w, c3_s inx_s, c3_y cod)
{
  if ( inx_s <= 0xFF ) {
...
_n_prog_asm(u3_noun ops, u3n_prog* pog_u, u3_noun sip)
{
  ...
  c3_s    lit_s  = 0,
          ...
          mem_s  = 0,
          reg_s  = 0;
  c3_w_tmp    i_w    = pog_u->byc_u.len_w-1;
  ...
  while ( i_w-- > 0 ) {
    ...
    if ( c3y == u3ud(op) ) {
      ...
      switch ( cod ) {
        ...
        case SWIP: case SWIN: {
          c3_m off_m   = u3t(op);
          buf_y[i_w--] = (c3_y) (off_m >> 24);
          buf_y[i_w--] = (c3_y) (off_m >> 16);
          buf_y[i_w--] = (c3_y) (off_m >> 8);
          buf_y[i_w--] = (c3_y) off_m;
          buf_y[i_w]   = (c3_y) cod;
          break;
        }
...
static c3_w_tmp _n_comp(u3_noun*, u3_noun, c3_o, c3_o);
...
_n_bint(u3_noun* ops, u3_noun hif, u3_noun nef, c3_o los_o, c3_o tel_o)
{
  ...
  if ( c3n == u3du(hif) ) {
    ...
    switch ( hif ) {
      ...
      case c3__bout: {
        u3_noun fen = u3_nul;
        c3_w_tmp  nef_w = _n_comp(&fen, nef, los_o, c3n);
 ...
 */
static inline c3_s
_n_resh(c3_y* buf, c3_w_tmp* ip_w)
{
  c3_y les = buf[(*ip_w)++];
  c3_y mos = buf[(*ip_w)++];
  return les | (mos << 8);
 ...
 */
static void
_n_print_byc(c3_y* pog, c3_w_tmp her_w)
{
  c3_w_tmp ip_w = 0;
  if ( her_w == 0 ) {
...
_cn_etch_bytecode(u3_noun fol) {
  ...
  c3_w_tmp len_w = pog_u->byc_u.len_w;
  c3_w_tmp ip_w=0, num_w=0, bop_w=0, dex_w=0;
  c3_w_tmp len_c = 1; // opening "{"
...
_cn_take_prog_cb(u3p(u3n_prog) pog_p)
{
  ...
  if ( c3y == pog_u->byc_u.own_o ) {
    c3_w_tmp pad_w = (8 - pog_u->byc_u.len_w % 8) % 8;
...
_n_ream(u3_noun kev)
{
  ...
  c3_w_tmp pad_w = (8 - pog_u->byc_u.len_w % 8) % 8;
  c3_w_tmp pod_w = pog_u->lit_u.len_w % 2;
  c3_w_tmp ped_w = pog_u->mem_u.len_w % 2;
...
{
  c3_n i_n, tot_n = u3a_mark_mptr(pog_u);

  for ( i_n = 0; i_n < pog_u->lit_u.len_w; ++i_n ) {
    tot_n += u3a_mark_noun(pog_u->lit_u.non[i_n]);
  }

  for ( i_n = 0; i_n < pog_u->mem_u.len_w; ++i_n ) {
    tot_n += u3a_mark_noun(pog_u->mem_u.sot_u[i_n].key);
  }

  for ( i_n = 0; i_n < pog_u->cal_u.len_w; ++i_n ) {
    tot_n += u3j_site_mark(&(pog_u->cal_u.sit_u[i_n]));
  }

  for ( i_n = 0; i_n < pog_u->reg_u.len_w; ++i_n ) {
    tot_n += u3j_rite_mark(&(pog_u->reg_u.rit_u[i_n]));
  }
